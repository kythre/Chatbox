...
function chatbox.init()
	...
	chatbox.textentry:SetPos( 0, chatbox.frame:GetTall()-chatbox.textentry:GetTall() )
	...
	chatbox.html:SetSize( chatbox.frame:GetWide()-5, chatbox.frame:GetTall()-40)
	chatbox.html:SetPos(0,20)
...
hook.Add( "ChatText", "kyle_chatbox_chattext", function( index, name, text, type )
	if type == "joinleave" or type == "none" then
		...
		chatbox.html:Call( "document.body.scrollTop = document.body.scrollHeight;")
	end
end )
...
if not GAMEMODE then
	...
	hook.Add("Initialize", "kyle_chatbox_init", function()
		include("autorun/cl_kyle_chatbox.lua")
		chatbox.init()
	end)
	else
	chatbox.init()
